...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        seq_length = tokens.numel() # total number of tokens
        eod_positions = (
        ...
        if eod_positions.numel() > 0:
            for pos in eod_positions:
                cu_seqlens_list.append(pos.item() + 1) # [start_pos, start_pos + length) +1 to convert from position to length
        # Extend last segment to cover remaining tokens (padding or partial doc) (as we didn't found an EOD token at the end)
        ...
        # adjacent_diffs = cu_seqlens[1:] - cu_seqlens[:-1] # lengths of each segment
        adjacent_diffs = torch.diff(cu_seqlens) # lengths of each segment
